...
if length(ARGS) < 4
    println("Usage: zero_field.jl geom N dim alpha rhos...")
    exit()
end
...
const ρs = length(ARGS) > 4 ? sort!(collect(Set(parse.(Float64, ARGS[5:end])))) : sort!([1.99, 0.5:0.05:1.95...])
const SHOTS = 100
...
@time begin
    ...
    logmsg("Running ED")
    eddata = ED.run_ed_parallel2(posdata, ALPHA, [0]; symmetry=ZBlockBasis(N, div(N-1,2))) # choose biggest block
    logmsg("Saving")
    ED.save(PREFIX, eddata; folder="zero_field")
    logmsg("Done!")
end
