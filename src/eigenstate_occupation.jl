...
load_eon(geometry, dimension, system_size, α, statename, location=SaveLocation(); prefix=location.prefix, suffix=location.suffix) = load(EONData(statename, geometry, dimension, system_size, α; prefix, suffix))
...
function ED.compute_task!(task::EONTask, ρindex, shot, fieldindex, eigen)
    for (i, vec) in enumerate(eachcol(eigen.vectors))
        task.data[i, fieldindex, shot, ρindex] = dot(vec, task.state)
    end
end
