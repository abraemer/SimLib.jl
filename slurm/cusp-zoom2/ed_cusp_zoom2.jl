...
#SBATCH --job-name=cusp-zoom
#SBATCH --output="logs/cusp-zoom-%j.out"
########### End Slurm header ##########
#=
# load modules
...
=#
println("ed_cusp_zoom.jl")

# check ARGS length and print usage if wrong
if length(ARGS) != 4
    println("Usage: ed_cusp_zoom.jl geom N dim alpha")
    exit()
end
...
## constants and ARGS
const PREFIX = joinpath(path_prefix(), "cusp-zoom2")
@show PREFIX
...
@time begin
    ...
    logmsg("Running ED")
    eddata = ED.run_ed_parallel2(posdata, ALPHA, FIELDS)
    logmsg("Saving")
